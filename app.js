...
bot.command('allresellerstats', async (ctx) => {
  try {
    ...
    const firstKeyboard = totalPages > 1
      ? { inline_keyboard: [[{ text: 'Next ', callback_data: 'allresellerstats_page_1' }]] }
      : null;
...
bot.action('admin_menu_markup', async (ctx) => {
  ...
  await ctx.editMessageText(text, { parse_mode: 'HTML', reply_markup: { inline_keyboard: [
    ...
    [{ text: '🟣 Akrab V3', callback_data: 'akrab_v3_reseller_markup_menu' },
     { text: '🔴 Circle',   callback_data: 'circle_reseller_markup_menu'  }],
...
bot.action('menu_ppob', async (ctx) => {
  ...
  const text =
    '<code>┌─────────────────────────┐</code>\n' +
    '<code>│</code> 🛒 <b>LAYANAN PPOB</b>\n' +
    '<code>│─────────────────────────</code>\n' +
    '<code>│</code> Saldo Tembak Kuota: <code>Rp ' + Number(saldoAkrab).toLocaleString('id-ID') + '</code>\n' +
    '<code>│─────────────────────────</code>\n' +
    '<code>│</code> 📱 Pulsa · 📶 Kuota Data\n' +
    '<code>│</code> 🎁 Combo Plus · 🔄 Masa Aktif\n' +
    '<code>│</code> 🆕 Aktivasi · ⚡ PLN · 🎮 Games\n' +
    '<code>└─────────────────────────┘</code>' + statusLine;
  try {
